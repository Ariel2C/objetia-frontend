...
export default function AppearanceTab({
  ...
  return (
    <div className="space-y-6 animate-fade-in font-sans text-[#d4d4d4] max-w-5xl">
      ...
      {tieneCambiosMarca && (
        <div className="flex justify-end pb-1 border-b border-[#262626]">
          <button 
      ...
      <div className="bg-[#1f1f1f] border border-[#2b2b2b] rounded-[16px] p-6 space-y-6">
        ...
        <div className="bg-[#18181a] border border-[#2b2b2b] rounded-xl p-5 flex flex-col sm:flex-row items-center justify-between gap-6">
          <div className="flex items-center gap-4">
            <div className="h-24 w-24 sm:h-28 sm:w-28 shrink-0 aspect-square flex items-center justify-center p-2.5 bg-[#121214] rounded-xl border border-[#2e2e32] shadow-inner overflow-hidden">
              {logoUrl ? (
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3.5">
            ...
            {/* Listado de Logos Cuadrados */}
            {uniqueLogos.map((logo) => {
              const esActivo = logoUrl === logo.logo_url;
              return (
                <div 
                  key={logo.id}
                  className={`relative aspect-square flex flex-col justify-between bg-[#18181a] border rounded-xl p-2.5 transition-all ${
                    esActivo 
                ...
                >
                  {/* Badge Activo */}
                  {esActivo && (
                    <span className="absolute top-2 right-2 px-1.5 py-0.5 text-[8px] font-bold bg-[#87a9ff] text-[#121214] rounded z-10">
                      ACTIVO
                    </span>
                  )}

                  <div className="flex-1 w-full flex items-center justify-center p-2 bg-[#121214] rounded-lg overflow-hidden border border-[#262626] min-h-0">
                  ...
                  <div className="w-full flex items-center gap-1.5 mt-2 shrink-0">
                    ...
                    >
                      ...
                      <span className="truncate">{esActivo ? 'Activo' : 'Elegir'}</span>
                    </button>

                    <button 
                      type="button"
                      onClick={() => handleDescargarLogo(logo.logo_url, `${logo.label || 'logotipo'}.png`)}
                      title="Descargar logotipo"
                      className="h-7 w-7 shrink-0 flex items-center justify-center rounded-lg border border-[#333333] bg-[#252525] text-[#8c8c8c] hover:text-[#87a9ff] hover:bg-[#87a9ff]/10 hover:border-[#87a9ff]/30 transition cursor-pointer"
                    >
                    ...
                    <button 
                      ...
                      className="h-7 w-7 shrink-0 flex items-center justify-center rounded-lg border border-[#333333] bg-[#252525] text-[#8c8c8c] hover:text-red-400 hover:bg-red-500/10 hover:border-red-500/30 transition cursor-pointer"
                    >
